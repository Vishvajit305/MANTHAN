...
def recognize_video():
    ...
    class camThreading(threading.Thread):
        ...
        def run(self):
            print ("Starting " + self.previewName)
            camafter(self.previewName, self.camID)
    ...
    def camPreview(previewName,camID):
        ...
        # otherwise, we are reading from a video file
            ...
            vs = cv2.VideoCapture(args["video"])# initialize the video stream, then allow the camera sensor to warm up
        ...
        while True:
            ...
            cv2.putText(frame,f"CCTV NO.{camID+1}",(0,25),cv2.FONT_HERSHEY_SIMPLEX,0.70,(0,0,0),2)
        ...
        vs.stop()
    def camafter(previewName,camID):
        import detect
        reload(detect)
...
def create_dataset():
    x=messagebox.askyesnocancel("Who is he!!","Is he a terrorist?!")
    if(x!=None):
        if(x==TRUE):
            import Terrorist_video
            reload(Terrorist_video)
        elif(x==False):
            import Batchmate_video
            reload(Batchmate_video)
windows=tk.Tk()
windows.title("Face Detection System")
windows.resizable(True,True)

button0=tk.Button(windows,text="Upload files",font=("Algerian",17),bg="white",fg="black", command=create_dataset)
